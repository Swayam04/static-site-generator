...
def generate_pages(dir_path_content : str, template_path : str, dest_dir_path : str) -> None:
    ...
    generate_pages_recursive(md_file_dir, template_html_file_path, generated_html_dir_path)
    
    
def generate_pages_recursive(md_path : Path, template_path : Path, dest_path : Path) -> None:
        try:
        ...
        for item in md_path.iterdir():
            ...
            if item.is_dir():
                ...
                final_html = template_html.replace("{{ Title }}", title).replace("{{ Content }}", html_content)
